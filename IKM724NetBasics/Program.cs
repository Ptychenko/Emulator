...
namespace AutomatedWaterTreatmentSystem
{
    class Program
    {
        ...
        private static WaterTreatmentSystem treatmentSystem = new WaterTreatmentSystem();
        ...
        static async Task Main(string[] args)
        {
            ...
            foreach(var indicator in deserializedResult)
            {
                if(sensorFactories.TryGetValue(indicator.Name, out var createSensor))
                {
                    ...
                    treatmentSystem.Sensors.Add(sensor);
                }
                else 
                {
        ...
        {
            treatmentSystem.Monitor();

            var sensor = treatmentSystem.Sensors.FirstOrDefault(x => x.Name == indicatorModel.Name);

            return sensor != null ? Math.Round(sensor.Value, 2) : 0.0;
        }
    }
}